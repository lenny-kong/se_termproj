...
*/		
		int cnt;
		String[] left_token = gui_data_model.getLeft_text_area().getText().split("\n");
		String[] right_token = gui_data_model.getRight_text_area().getText().split("\n");

		gui_data_model.getLeft_list().clear();
		gui_data_model.getRight_list().clear();
		// delete before data in left or right list

		for (cnt = 0; cnt <left_token.length; cnt++) {
			gui_data_model.getLeft_list().add(new String_object(left_token[cnt]));
		}
		for (cnt = 0; cnt <right_token.length; cnt++) {
			gui_data_model.getRight_list().add(new String_object(right_token[cnt]));
		}
		// put data in list
	}
	...
	public void print_list_text_area() {
		...
		// delete all contents in left and right text area to show compare list in gui
		// after algorithm
		...
		for (cnt = 0; cnt < gui_data_model.getLeft_list().size(); cnt++) {
			...
			if (cnt != gui_data_model.getLeft_list().size() - 1)
		...
		for (cnt = 0; cnt < gui_data_model.getRight_list().size(); cnt++) {
			...
			if (cnt != gui_data_model.getRight_list().size() - 1)
